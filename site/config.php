 ...
 */
$kb->config['debug']['kobayashi'] = false;
$kb->config['debug']['session'] = false;
$kb->config['debug']['timer'] = true;
$kb->config['debug']['db-num-queries'] = true;
$kb->config['debug']['db-queries'] = true;


/**
 * Set database(s).
 */
$kb->config['database'][0]['dsn'] = 'sqlite:' . KOBAYASHI_SITE_PATH . '/data/.ht.sqlite';
 ...
 */
$kb->config['url_type'] = 1;


/**
 * Set a base_url to use another than the default calculated
 */
$kb->config['base_url'] = null;


/**
 * How to hash password of new users, choose from: plain, md5salt, md5, sha1salt, sha1.
 */
$kb->config['hashing_algorithm'] = 'sha1salt';


/**
 * Allow or disallow creation of new user accounts.
 */
$kb->config['create_new_users'] = true;


/**
 * Define session name
 */
$kb->config['session_name'] = preg_replace('/[:\.\/-_]/', '', __DIR__);
$kb->config['session_key']  = 'kobayashi';


/**
 * Define default server timezone when displaying date and times to the user. All internals are still UTC.
 */
$kb->config['timezone'] = 'Europe/Stockholm';


/**
 * Define internal character encoding
 */
$kb->config['character_encoding'] = 'UTF-8';


/**
 * Define language
 */
$kb->config['language'] = 'en';
 ...
 * Define menus.
 *
 * Create hardcoded menus and map them to a theme region through $kb->config['theme'].
 */
$kb->config['menus'] = array(
  'navbar' => array(
 ...
 * Template files can reside in the parent or current theme, the CKobayashi::ThemeEngineRender()
 ...
 *
 * path: Path to current theme, relativly KOBAYASHI_INSTALL_PATH, for example themes/grid or site/themes/mytheme.
...
$kb->config['theme'] = array(
  ...
  'data' => array(
    'header' => 'Kobayashi Maru',
    'slogan' => 'A PHP-based MVC-inspired CMF based on Lydia',
    'favicon' => 'logo_80x80.png',
    'logo' => 'logo_80x80.png',
    'logo_width'  => 80,
    'logo_height' => 80,
    'footer' => '<p>Kobayashi Maru, based on Lydia &copy; by Mikael Roos (mos@dbwebb.se)</p>',
  ),
);
