<?php
/**
 * A model for managing Kobayashi Maru modules.
 * 
...
class CMModules extends CObject {
   ...
   * Properties
   */
  private $kobayashiCoreModules = array('CKobayashi', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $kobayashiCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  public function ReadAndAnalyse() {
    $src = KOBAYASHI_INSTALL_PATH.'/src';
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isKobayashiCore']   = in_array($rc->name, $this->kobayashiCoreModules);
      $details['isKobayashiCMF']    = in_array($rc->name, $this->kobayashiCMFModules);
