<?php
/**
 * Main class for Kobayashi Maru, holds everything.
 *
 * @package KobayashiCore
 */
class CKobayashi implements ISingleton {
	...
	protected function __construct() {
		...
		// include the site specific config.php and create a ref to $kb to be used by config.php
		$kb = &$this;
    require(KOBAYASHI_SITE_PATH.'/config.php');
  ...
  /**
	 ...
	 * @return CKobayashi The instance of this class.
	 */
	public static function Instance() {
		if(self::$instance == null) {
			self::$instance = new CKobayashi();
		}
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= KOBAYASHI_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = KOBAYASHI_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $kb->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $kb
    ...
    $kb = &$this;
    // First the default Kobayashi themes/functions.php
    include(KOBAYASHI_INSTALL_PATH . '/themes/functions.php');
    ...
    // Extract $kb->data to own variables and handover to the template file
  ...
  /**
   * Draw HTML for a menu defined in $kb->config['menus'].
   *
