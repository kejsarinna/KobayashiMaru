...
function autoload($aClassName) {
  $classFile = "/src/{$aClassName}/{$aClassName}.php";
	$file1 = KOBAYASHI_SITE_PATH . $classFile;
	$file2 = KOBAYASHI_INSTALL_PATH . $classFile;
	if(is_file($file1)) {
		require_once($file1);
...
function exceptionHandler($e) {
  echo "Kobayashi: Uncaught exception: <p>" . $e->getMessage() . "</p><pre>" . $e->getTraceAsString(), "</pre>";
}
...
function htmlEnt($str, $flags = ENT_COMPAT) {
  return htmlentities($str, $flags, CKobayashi::Instance()->config['character_encoding']);
}
